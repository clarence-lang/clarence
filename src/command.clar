...
; Allow clar to emit Node.js events
...
; Run `clar` by parsing passed options and branching on actions
(= exports.run (def run
  (do (parseOptions)
...
; Compiles a path, which could be a script or directory. If a directory is passed, recursively compile all '.clar' files and subdirectories
(def compilePath source topLevel base
  (do (if (or (in source sources)
      ...
      (if (stats.isDirectory)
          ...
          (elif (or topLevel (utils.isclar source))
...
(def findDirectoryIndex source
     (do (for ext clar.fileExtensions
  ...
  (do (= o       opts
      ...
      (try
        (do (= t (= task (file: file input: input options: options)))
            ...
            (if o.run
                (do (clar.register)
                    (clar.run t.input t.options))
                (do (= compiled (clar.compile t.input)
                       t.output compiled)
        ...
        (catch err
          (do (clar.emit "failure" err task)
              (if (clar.listeners "failure").length (return))
...
(def version
     (printLine (+ "clarence version " clar.version)))